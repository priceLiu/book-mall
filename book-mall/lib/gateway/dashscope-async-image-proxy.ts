...
/** 百炼 image-erase-completion 默认 add_watermark=true，会烧左下角 Generated by AI */
function withDefaultNoVendorWatermark(
  model: string,
  parameters?: Record<string, unknown>,
): Record<string, unknown> {
  const out = { ...(parameters ?? {}) };
  if (model.trim() === "image-erase-completion" && out.add_watermark !== true) {
    out.add_watermark = false;
  }
  return out;
}
...
> {
  const created = await dashscopeCreateImage2ImageTask({
    ...
    model: opts.model,
    input: opts.input,
    parameters: withDefaultNoVendorWatermark(opts.model, opts.parameters),
  });
  if (!created.ok) return created;
  return dashscopePollTaskUntilDone({
    apiKey: opts.apiKey,
