/**
 * 百炼 OpenAI 兼容 Chat 模型（经 Gateway BAILIAN 凭证路由）。
 * 模型 ID 与 DashScope compatible-mode /v1/chat/completions 一致。
 */

import type { CanvasGatewayListedModel } from "@/lib/canvas/providers/types";

const LLM_TEMP_SCHEMA = [
  {
    key: "temperature",
    label: "temperature",
    type: "number" as const,
    min: 0,
    max: 2,
    step: 0.1,
    defaultValue: 0.7,
  },
  {
    key: "max_tokens",
    label: "max_tokens",
    type: "number" as const,
    min: 256,
    max: 32000,
    step: 128,
    defaultValue: 8000,
  },
] as const;

export const BAILIAN_CHAT_KNOWN_MODELS: CanvasGatewayListedModel[] = [
  {
    modelKey: "qwen3.8-max",
    displayName: "Qwen3.8-Max",
    role: "LLM",
    description: "百炼 · 旗舰 All-in-One · 文本 / 思考 / 图片理解 / 长视频理解",
    paramsSchema: [...LLM_TEMP_SCHEMA],
    defaultParams: { temperature: 0.7, max_tokens: 8000 },
  },
  {
    modelKey: "qwen3-omni-flash",
    displayName: "Qwen3-Omni Flash",
    role: "LLM",
    description: "百炼 · Omni 全模态 · 文本 / 图片 / 音频 / 视频理解",
    paramsSchema: [...LLM_TEMP_SCHEMA],
    defaultParams: { temperature: 0.7, max_tokens: 8000 },
  },
  {
    modelKey: "qwen2.5-vl-72b-instruct",
    displayName: "Qwen2.5-VL 72B",
    role: "LLM",
    description: "百炼 · 2.5-VL Max · 图片 / 视频理解",
    paramsSchema: [...LLM_TEMP_SCHEMA],
    defaultParams: { temperature: 0.7, max_tokens: 8000 },
  },
  {
    modelKey: "glm-5.3-flash",
    displayName: "GLM-5.3 Flash",
    role: "LLM",
    description: "百炼 · 智谱 GLM-5.3 Flash · 图片 / 视频 / 文件理解 · 深度思考",
    paramsSchema: [
      {
        key: "max_tokens",
        label: "max_tokens",
        type: "number" as const,
        min: 256,
        max: 131072,
        step: 256,
        defaultValue: 8000,
      },
      {
        key: "reasoning_effort",
        label: "思考深度",
        type: "select" as const,
        options: [
          { value: "max", label: "Max · 深度推理（默认）" },
          { value: "high", label: "High · 增强推理" },
          { value: "low", label: "Low · 轻度推理" },
        ],
        defaultValue: "max",
        help: "GLM-5.3 Flash 始终开启思考；temperature 由模型固定。",
      },
    ],
    defaultParams: { max_tokens: 8000, reasoning_effort: "max", enable_thinking: true },
  },
  {
    modelKey: "qwen3.7-plus",
    displayName: "Qwen3.7-Plus",
    role: "LLM",
    description: "百炼 · 旗舰 · 视频/图片理解 · 反推提示词",
    paramsSchema: [...LLM_TEMP_SCHEMA],
    defaultParams: { temperature: 0.7, max_tokens: 8000 },
  },
  {
    modelKey: "qwen3.6-plus",
    displayName: "Qwen3.6-Plus",
    role: "LLM",
    description: "百炼 · 多模态 · 视频/图片理解 · 反推提示词",
    paramsSchema: [...LLM_TEMP_SCHEMA],
    defaultParams: { temperature: 0.7, max_tokens: 8000 },
  },
  {
    modelKey: "qwen3.5-plus",
    displayName: "Qwen3.5-Plus",
    role: "LLM",
    description: "百炼 · 效果优先 · 视频/图片理解",
    paramsSchema: [...LLM_TEMP_SCHEMA],
    defaultParams: { temperature: 0.7, max_tokens: 8000 },
  },
  {
    modelKey: "qwen3-vl-plus",
    displayName: "Qwen3-VL-Plus",
    role: "LLM",
    description: "百炼 · 多模态 · 视频理解 · 反推提示词推荐",
    paramsSchema: [...LLM_TEMP_SCHEMA],
    defaultParams: { temperature: 0.7, max_tokens: 8000 },
  },
  {
    modelKey: "qwen3.5-27b",
    displayName: "Qwen3.5-27B",
    role: "LLM",
    description: "百炼 · 均衡性价比 · 提示词优化推荐",
    paramsSchema: [...LLM_TEMP_SCHEMA],
    defaultParams: { temperature: 0.7, max_tokens: 8000 },
  },
  {
    modelKey: "qwen3.5-flash",
    displayName: "Qwen3.5-Flash",
    role: "LLM",
    description: "百炼 · 快速经济",
    paramsSchema: [...LLM_TEMP_SCHEMA],
    defaultParams: { temperature: 0.7, max_tokens: 8000 },
  },
  {
    modelKey: "qwen-plus",
    displayName: "Qwen Plus",
    role: "LLM",
    description: "百炼 · 经典均衡模型",
    paramsSchema: [...LLM_TEMP_SCHEMA],
    defaultParams: { temperature: 0.7, max_tokens: 2000 },
  },
  {
    modelKey: "qwen-max",
    displayName: "Qwen Max",
    role: "LLM",
    description: "百炼 · 效果优先（耗时更长）",
    paramsSchema: [...LLM_TEMP_SCHEMA],
    defaultParams: { temperature: 0.7, max_tokens: 8000 },
  },
  {
    modelKey: "MiniMax/MiniMax-M2.7",
    displayName: "MiniMax M2.7",
    role: "LLM",
    description: "百炼 · MiniMax 旗舰 · 编程/摘要 · 提示词优化推荐",
    paramsSchema: [
      {
        key: "max_tokens",
        label: "max_tokens",
        type: "number" as const,
        min: 256,
        max: 32000,
        step: 128,
        defaultValue: 8000,
      },
    ],
    defaultParams: { max_tokens: 8000 },
  },
  {
    modelKey: "MiniMax/MiniMax-M2.5",
    displayName: "MiniMax M2.5",
    role: "LLM",
    description: "百炼 · MiniMax M2.5（兼容保留）",
    paramsSchema: [
      {
        key: "max_tokens",
        label: "max_tokens",
        type: "number" as const,
        min: 256,
        max: 32000,
        step: 128,
        defaultValue: 8000,
      },
    ],
    defaultParams: { max_tokens: 8000 },
  },
  {
    modelKey: "MiniMax-M2.5",
    displayName: "MiniMax M2.5 Legacy Alias",
    role: "LLM",
    description: "百炼 · 历史 modelKey 别名，等同 MiniMax/MiniMax-M2.5",
    paramsSchema: [
      {
        key: "max_tokens",
        label: "max_tokens",
        type: "number" as const,
        min: 256,
        max: 32000,
        step: 128,
        defaultValue: 8000,
      },
    ],
    defaultParams: { max_tokens: 8000 },
  },
  {
    modelKey: "kimi-k3",
    displayName: "Kimi K3",
    role: "LLM",
    description: "百炼 · 旗舰 · 1M 上下文 · 剧本 / 长文推荐",
    paramsSchema: [
      {
        key: "max_tokens",
        label: "max_tokens",
        type: "number" as const,
        min: 256,
        max: 131072,
        step: 256,
        defaultValue: 16384,
      },
    ],
    defaultParams: { max_tokens: 16384 },
  },
  {
    modelKey: "kimi-k2.6",
    displayName: "Kimi K2.6",
    role: "LLM",
    description: "百炼 · 256K 上下文 · 可选深度思考 · 剧本创作",
    paramsSchema: [
      {
        key: "max_tokens",
        label: "max_tokens",
        type: "number" as const,
        min: 256,
        max: 131072,
        step: 256,
        defaultValue: 16384,
      },
      {
        key: "thinking_mode",
        label: "深度思考",
        type: "select" as const,
        options: [
          { value: "enabled", label: "开启（默认）" },
          { value: "disabled", label: "关闭 · 更快" },
        ],
        defaultValue: "enabled",
        help: "Kimi K2.6 专有；temperature 由模型固定，勿手动传入。",
      },
    ],
    defaultParams: { max_tokens: 16384, thinking_mode: "enabled" },
  },
  {
    modelKey: "kimi-k2.7-code",
    displayName: "Kimi K2.7 Code",
    role: "LLM",
    description: "百炼 · 256K · 代码 / 结构化剧本 · 始终深度思考",
    paramsSchema: [
      {
        key: "max_tokens",
        label: "max_tokens",
        type: "number" as const,
        min: 256,
        max: 131072,
        step: 256,
        defaultValue: 16384,
      },
    ],
    defaultParams: { max_tokens: 16384 },
  },
];
