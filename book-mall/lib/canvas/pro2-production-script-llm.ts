...
export function buildPro2StructuredRetryUserMessage(
  ...
  return [
    ...
    "",
    "【对白 dialogue · 若校验错误含对白/dialogue 须优先修正】",
    ...
    "6. **imagePrompt 字面量硬性**：scenes[] / characters[] / props[] 每条 imagePrompt 必须同时含「构图规范」与「[视觉风格：…]」；props 不得省略 imagePrompt",
    "7. characters[].traits ≥3 项；禁止「标志性动作」",
    "8. **dialogue**：非 — 时须 角色名（情绪）：\"台词\"（ASCII 直引号或「」）；禁止无引号/弯引号/「角色说：」；且 **characterIds 非空**",
