...
async function seedCatalogsFromToolBillablePrices(): Promise<{
  ...
  const prices = await prisma.toolBillablePrice.findMany({
    where: {
      active: true,
      schemeARefModelKey: { not: null },
      cloudBillingKind: { not: null },
    },
    select: {
      toolKey: true,
  ...
  for (const p of prices) {
    const canonicalKey = p.schemeARefModelKey?.trim();
    if (!canonicalKey) continue;
    if (!p.cloudBillingKind) continue;
    ...
    if (existing) {
      ...
      const fresh = await prisma.modelCatalog.create({
        data: {
          canonicalKey,
          displayName: displayNameFromCanonical(canonicalKey, p.toolKey),
          vendor: vendorFromToolKey(p.toolKey),
          billingKind: p.cloudBillingKind,
          unitLabel: UNIT_LABEL_BY_KIND[p.cloudBillingKind],
          defaultTierRaw: p.cloudTierRaw ?? null,
          active: true,
        },
        select: { id: true },
      });
    ...
    // 把 (INTERNAL_SCHEME_A_MODEL, canonicalKey) 挂到 catalog（用作工具站侧反查的入口）
    ...
    if (!aliasExisting) {
      await prisma.modelAlias.create({
        data: {
          ...
          matchedBy: "seed:tool_billable_price",
        },
      });
      aliasAttached++;
