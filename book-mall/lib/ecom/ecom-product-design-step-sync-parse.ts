import type { ProductDesign, ProductDesignMainImage } from "@/lib/ecom/ecom-product-design-types";
...
import { isStep3ReadyForDownstream } from "@/lib/ecom/ecom-product-design-buying-reason-parse";
import { sanitizeMiddlePanelMarkdown } from "@/lib/ecom/ecom-product-design-middle-sanitize";
...
export function coalesceMainImagesFromText(
  ...
  const jsonItems = extractProductDesignJson(markdownText)?.mainImages ?? [];
...
export function coalesceDetailOutlineFromText(
  ...
  const jsonItems = extractProductDesignJson(markdownText)?.detailOutline ?? [];
...
function scoreStep4AssistantMessage(content: string): number {
  ...
  const jsonItems = extractProductDesignJson(content)?.mainImages ?? [];
...
function scoreStep7AssistantMessage(content: string): number {
  ...
  const jsonItems = extractProductDesignJson(content)?.detailOutline ?? [];
...
export function parseAnalysisFromMarkdown(text: string): ProductAnalysis | null {
  const json = extractProductDesignJson(text);
  if (json?.analysis && hasValidAnalysis(normalizeAnalysis(json.analysis))) {
    return normalizeAnalysis(json.analysis);
  }
