...
export async function ecomGenerateFilmPullShotsBatch(opts: {
  ...
  const project = await getEcomFilmPullProject(opts.userId, opts.projectId);
  const plan = project?.renderPlan?.shots ?? [];
  const targets =
...
export async function ecomRenderFilmPullFinalVideo(opts: {
  ...
  const { fromEcomFilmPullPlan } = await import("@/lib/ecom/adapters/ecom-film-pull-timeline");
  ...
  const timeline = fromEcomFilmPullPlan(plan);
  ...
  for (let i = 0; i < 120; i++) {
    ...
    if (row?.status === "SUCCEEDED" && row.resultOssUrl?.trim()) {
      ...
      await updateEcomFilmPullProject(opts.userId, opts.projectId, {
        ...
        renderPlan: {
          ...plan,
          render: { jobId: job.id, finalVideoUrl },
        },
        meta: {
          ...(project?.meta ?? {}),
          finalVideoUrl,
