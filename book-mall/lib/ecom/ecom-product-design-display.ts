...
export function formatAnalysisChatMarkdown(
  ...
  const lines: string[] = [
    ...
    "",
    "### 平台浏览习惯与文案红线",
    analysis.platformNotes.trim() || "（见左侧工作区）",
    "",
    "### 表层痛点",
...
export function formatMainImagesChatMarkdown(
  items: ProductDesign["mainImages"],
): string {
  const lines: string[] = [
    `## Step4 · 主图分层文案（${items.length} 张定稿）`,
    "",
    "各张主图分层文案已写入中间工作区，可在左侧逐张查看与编辑；确认后点 **下一步** 进入详情页架构。",
    "",
  ];
...
/** 助手气泡展示文案：优先 Markdown，纯 JSON 围栏时生成可读摘要 */
