...
import { ECOM_DETAIL_PAGE_SUITE_HIT_TOOL_KEY } from "@/lib/ecom/detail-page-suite/types";
...
export class HitVisionSellpointAlreadyRunningError extends Error {
  constructor() {
    super("识图卖点已在进行中，请稍候刷新进度");
    this.name = "HitVisionSellpointAlreadyRunningError";
  }
}
...
async function patchVisionJob(
  ...
  const updated = await updateDetailPageSuiteHitProject(userId, projectId, {
    meta: {
      ...(project.meta ?? {}),
      hitVisionSellpoint: job,
    },
  });
...
export async function beginHitVisionSellpointJob(opts: {
  userId: string;
  projectId: string;
}) {
  const project = await getDetailPageSuiteHitProject(opts.userId, opts.projectId);
  if (!project) throw new Error("项目不存在");
  if (project.meta?.hitVisionSellpoint?.status === "running") {
    throw new HitVisionSellpointAlreadyRunningError();
  }
  if (productUrls(project).length === 0) {
    throw new Error("请先上传至少 1 张新品产品图");
  }
...
export async function runHitVisionSellpointPipeline(opts: {
  userId: string;
  projectId: string;
  modelKey?: string;
}) {
  ...
  try {
    ...
    const sellPoints = await extractSellpointsFromProductImages({
      ...
      toolKey: ECOM_DETAIL_PAGE_SUITE_HIT_TOOL_KEY,
      clientPageSuffix: `${ECOM_DETAIL_PAGE_SUITE_HIT_TOOL_KEY}__vision`,
      productUrls: urls,
...
/** 同步整段执行（测试或 async:false） */
export async function visionSellpointsForHitProjectSync(opts: {
  userId: string;
  projectId: string;
  modelKey?: string;
}) {
  await beginHitVisionSellpointJob(opts);
  return runHitVisionSellpointPipeline(opts);
}
