...
把原「积分长期不清零」改为**按来源分别到期**：

| 来源 `CreditSource` | 到期口径 | 说明 |
|---|---|---|
| `SUBSCRIPTION` 订阅赠送 | **按月清零**（`expiresAt = currentPeriodEnd`，月度重置按 `periodKey` 换发） | 年付套餐同样按月刷新 |
...
| `POST /api/admin/credits/monthly-reset` | 每日 00:30 | `runMonthlyResetSweep`：`currentPeriodEnd<=now` 的订阅账户按月刷新 |
