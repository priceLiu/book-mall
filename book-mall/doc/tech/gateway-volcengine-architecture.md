...
| `lib/gateway/gateway-submit-error-policy.ts` | submit 错误分类、有限重试、failCode（全站复用） |
| `lib/gateway/volcengine-jobs.ts` | submit + poll 写日志 |
| `doc/tech/gateway-video-task-handling.md` | 视频任务失败原因、策略与两轮测试计划 |
