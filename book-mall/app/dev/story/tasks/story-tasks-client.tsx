...
type StoryTask = {
  ...
  frameId: string | null;
  kind: "COVER_IMAGE" | "CHARACTER_AVATAR" | "FRAME_IMAGE" | "FRAME_VIDEO";
...
const KIND_TABS: { id: "ALL" | StoryTask["kind"]; label: string }[] = [
  { id: "ALL", label: "全部" },
  { id: "COVER_IMAGE", label: "封面" },
  { id: "CHARACTER_AVATAR", label: "角色头像" },
  { id: "FRAME_IMAGE", label: "分镜图" },
  { id: "FRAME_VIDEO", label: "分镜视频" },
];

const STATUS_OPTS: { id: "ALL" | StoryTask["status"]; label: string }[] = [
  { id: "ALL", label: "全部状态" },
...
function statusColor(s: StoryTask["status"]): { dot: string; text: string } {
  switch (s) {
    case "SUCCEEDED":
    ...
    case "SUBMITTED":
      return { dot: "bg-sky-400 animate-pulse", text: "text-sky-300" };
    case "PENDING":
    default:
...
function formatDuration(t: StoryTask): string {
  const start = t.submittedAt ?? t.createdAt;
  const end = t.completedAt ?? (t.status === "SUBMITTED" ? new Date().toISOString() : null);
  if (!start || !end) return "—";
...
function ResultButton({ task }: { task: StoryTask }) {
...
export function StoryTasksClient() {
  ...
  const [activeKind, setActiveKind] = useState<"ALL" | StoryTask["kind"]>("ALL");
  const [activeStatus, setActiveStatus] = useState<"ALL" | StoryTask["status"]>(
    "ALL",
  );
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const r = await fetch(`/api/dev/story/tasks?${params.toString()}`, {
        cache: "no-store",
      });
  ...
  // 自动刷新（5 秒），存在 SUBMITTED/PENDING 时才轮询
  useEffect(() => {
  ...
  const triggerPoll = useCallback(async () => {
    setPolling(true);
    try {
      const r = await fetch("/api/dev/story/tasks/poll", { method: "POST" });
        ...
        <div className="mx-auto flex max-w-[1400px] items-center gap-4 px-4 py-3 sm:px-6">
          ...
          <div className="min-w-0 flex-1">
            <h1 className="text-sm font-semibold text-white sm:text-base">
              KIE 任务（StoryGenerationTask）
            </h1>
            <p className="mt-0.5 text-[11px] text-zinc-500">
              共 {totalAll} 条 · 上次刷新 {fetchedAt ? new Date(fetchedAt).toLocaleTimeString("zh-CN") : "—"}
            </p>
          </div>
          <button
            type="button"
          ...
          <button
            ...
            disabled={polling}
            className="inline-flex items-center gap-1.5 rounded-md bg-sky-500 px-3 py-1.5 text-xs font-medium text-zinc-950 shadow-sm transition hover:bg-sky-400 disabled:opacity-60"
          >
            {polling ? (
        ...
        <div className="mx-auto flex max-w-[1400px] flex-wrap items-center gap-2 border-t border-zinc-800/70 px-4 py-2 sm:px-6">
          ...
          {pollResult ? (
            <span className="ml-auto text-[11px] text-zinc-500">
              上次回调 {new Date(pollResult.ranAt).toLocaleTimeString("zh-CN")}：
          ...
          <div className="overflow-hidden rounded-xl border border-zinc-800 bg-zinc-900/40">
            <table className="w-full text-left text-xs">
              <thead className="bg-zinc-900/80 text-[11px] uppercase tracking-wider text-zinc-500">
                <tr>
                  <th className="w-[200px] px-3 py-2.5 font-medium">模型 / 时间</th>
                  <th className="w-[260px] px-3 py-2.5 font-medium">参数</th>
                  <th className="w-[110px] px-3 py-2.5 font-medium">状态</th>
                  <th className="w-[120px] px-3 py-2.5 font-medium">耗时 · Poll</th>
                  <th className="w-[200px] px-3 py-2.5 font-medium">任务 ID</th>
                  <th className="w-[200px] px-3 py-2.5 font-medium">项目</th>
                    ...
                    <Fragment key={t.id}>
                      ...
                      >
                        ...
                        <td className="px-3 py-2.5 align-top text-[11px] text-zinc-400">
                          ...
                          <p className="text-[10px] text-zinc-600">poll {t.pollCount}</p>
                        </td>
                        <td className="px-3 py-2.5 align-top">
                          {t.kieTaskId ? (
                            <div className="flex items-center gap-1">
                              ...
                              <CopyButton value={t.kieTaskId} label="复制 KIE taskId" />
                            </div>
                          ) : (
                        ...
                        <td className="px-3 py-2.5 align-top">
                          {t.project ? (
                            <Link
                              href={`/api/story/projects/${t.project.id}`}
                        ...
                        <tr className="bg-zinc-950/70">
                          <td colSpan={7} className="px-3 py-3">
                            <div className="grid gap-3 lg:grid-cols-2">
                              ...
                              <div className="lg:col-span-2 grid gap-2 text-[11px] text-zinc-400 sm:grid-cols-3">
                                <div>
                                  <span className="text-zinc-500">submittedAt：</span>
                                  {formatDateTime(t.submittedAt)}
                                </div>
                                <div>
                                  <span className="text-zinc-500">completedAt：</span>
                                  {formatDateTime(t.completedAt)}
                                </div>
                                <div>
                                  <span className="text-zinc-500">lastPolledAt：</span>
                                  {formatDateTime(t.lastPolledAt)}
                                </div>
                                {t.ossUrl ? (
                                  <div className="sm:col-span-3">
                                    <span className="text-zinc-500">ossUrl：</span>
                                    <a
                                      href={t.ossUrl}
                                      target="_blank"
                                      rel="noopener noreferrer"
                                ...
                                {t.ephemeralUrl ? (
                                  <div className="sm:col-span-3">
                                    <span className="text-zinc-500">ephemeralUrl：</span>
                                    <a
                                      href={t.ephemeralUrl}
                                      target="_blank"
                                      rel="noopener noreferrer"
