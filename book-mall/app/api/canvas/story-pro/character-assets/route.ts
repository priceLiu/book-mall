...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const assets = await listStoryProCharacterAssets(guard.user.id, {
      projectId,
    });
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const asset = await upsertStoryProCharacterAssetRef(guard.user.id, {
      characterKey: String(body.characterKey ?? ""),
      displayName: String(body.displayName ?? ""),
      projectId: (body.projectId as string | null | undefined) ?? null,
      kind: (body.kind as StoryProCharacterAssetRefKind | undefined) ?? "three_view",
