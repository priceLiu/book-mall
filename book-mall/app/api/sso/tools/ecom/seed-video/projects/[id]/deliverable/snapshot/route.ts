...
import { persistSeedVideoDeliverableSnapshot } from "@/lib/ecom/ecom-seed-video-snapshot";
...
export async function POST(req: Request, ctx: Ctx) {
  ...
  try {
    ...
    const snapshot = await persistSeedVideoDeliverableSnapshot({
      userId: auth.userId,
      projectId,
      workName,
    });
    const refreshed = await getEcomSeedVideoProject(auth.userId, projectId);
