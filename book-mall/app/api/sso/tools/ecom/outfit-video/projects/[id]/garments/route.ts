...
import {
  patchEcomOutfitVideoGarments,
  uploadEcomOutfitVideoGarment,
...
export async function POST(req: Request, ctx: Ctx) {
  ...
  try {
    const project = await uploadEcomOutfitVideoGarment(auth.userId, id, kind, file, {
      fullSetSlot: slot,
...
export async function PATCH(req: Request, ctx: Ctx) {
  ...
  try {
    const project = await patchEcomOutfitVideoGarments(auth.userId, id, body);
    return NextResponse.json({ project });
  } catch (e) {
