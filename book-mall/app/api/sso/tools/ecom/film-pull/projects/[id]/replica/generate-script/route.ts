...
import { generateFilmPullReplicaScript } from "@/lib/ecom/ecom-film-pull-replica";
...
export async function POST(req: Request, ctx: Ctx) {
  ...
  try {
    ...
    const result = await generateFilmPullReplicaScript(auth.userId, id, {
      productBrief,
      modelKey,
    });
    return NextResponse.json(result);
  } catch (e) {
    const { message, status } = mapReplicaRouteError(e, "脚本生成失败");
