...
import { persistMediaDecomposeDeliverableSnapshot } from "@/lib/ecom/ecom-media-decompose-snapshot";
...
export async function POST(req: Request, ctx: Ctx) {
  ...
  try {
    ...
    const snapshot = await persistMediaDecomposeDeliverableSnapshot({
      userId: auth.userId,
      projectId,
      workName,
    });
    const refreshed = await getEcomMediaDecomposeProject(auth.userId, projectId);
