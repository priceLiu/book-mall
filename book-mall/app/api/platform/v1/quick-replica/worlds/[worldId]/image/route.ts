...
import { proxyWorldImageAsset } from "@/lib/quick-replica/qr-world-image-proxy";
...
export async function GET(request: Request, ctx: Ctx) {
  ...
  try {
    return await proxyWorldImageAsset({
      userId: auth.userId,
      worldId,
      upstreamUrl,
    });
    ...
    const message = e instanceof Error ? e.message : "image_proxy_failed";
