...
/**
 * 从 OpenArt 参考图下载预览、上传 OSS，并生成 builtin-image-gallery.json。
 *
 * 准备：book-mall/content/quick-replica/image-gallery-seed.json
 * 环境：book-mall/.env.local 中 OSS_* / OSS_PUBLIC_URL_BASE
 *
 * 使用：
 *   cd book-mall && pnpm qr:sync-image-gallery
 *   pnpm qr:sync-image-gallery --dry-run
 */
...
const SEED_JSON = resolve(ROOT, "content", "quick-replica", "image-gallery-seed.json");
const OUT_JSON = resolve(ROOT, "content", "quick-replica", "builtin-image-gallery.json");
const URLS_JSON = resolve(ROOT, "content", "quick-replica", "builtin-image-gallery.urls.json");
...
async function main() {
  ...
  if (templates.length === 0) {
    console.error("[sync-quick-replica-image-gallery] no templates generated");
    process.exit(1);
  }
  ...
  console.log(
    `[sync-quick-replica-image-gallery] done · seed=${seed.length} saved=${templates.length} → ${OUT_JSON}`,
  );
}
