...
/**
 * 从 OpenArt 下载运动同步预览图 + mp4、上传 OSS，并生成 builtin-motion-sync-gallery.json。
 *
 * 准备：book-mall/content/quick-replica/motion-sync-gallery-seed.json
 * 环境：book-mall/.env.local 中 OSS_* / OSS_PUBLIC_URL_BASE
 *
 * 使用：
 *   cd book-mall && pnpm qr:sync-motion-sync-gallery
 *   pnpm qr:sync-motion-sync-gallery --dry-run
 */
...
const SEED_JSON = resolve(ROOT, "content", "quick-replica", "motion-sync-gallery-seed.json");
const OUT_JSON = resolve(ROOT, "content", "quick-replica", "builtin-motion-sync-gallery.json");
const URLS_JSON = resolve(
  ROOT,
  "content",
  "quick-replica",
  "builtin-motion-sync-gallery.urls.json",
);
...
async function main() {
  ...
  if (templates.length === 0) {
    console.error("[sync-quick-replica-motion-sync-gallery] no templates generated");
    process.exit(1);
  }
  ...
  console.log(
    `[sync-quick-replica-motion-sync-gallery] done · seed=${seed.length} saved=${templates.length} → ${OUT_JSON}`,
  );
}
