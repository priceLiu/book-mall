...
/**
 * 从 OpenArt 参考图下载场景预览、上传 OSS，并生成 builtin-world-gallery.json。
 *
 * 准备：book-mall/content/quick-replica/world-gallery-seed.json
...
const SEED_JSON = resolve(ROOT, "content", "quick-replica", "world-gallery-seed.json");
...
async function main() {
  ...
  console.log(
    `[sync-quick-replica-world-gallery] done · seed=${seed.length} saved=${templates.length} → ${OUT_JSON}`,
  );
}
