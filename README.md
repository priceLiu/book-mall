...
├── finance-web/          # 财务控制台（默认 http://localhost:3002）
├── story-web/            # 漫剧个人空间（默认 http://localhost:3003）
├── scripts/
...
cd finance-web && pnpm install && cd ..
cd story-web && pnpm install && cd ..
```
...
主站与工具站已含 Dockerfile。**云托管 CloudBase Run**：同一仓库建四个服务，**目标目录**分别填 `book-mall`、`tool-web`、`finance-web`、`story-web`，端口 **3000** / **3001** / **3002** / **3003**。详见 **`deploy/tencent/README.md`**。
