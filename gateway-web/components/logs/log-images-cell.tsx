...
function LogImageThumb({ item }: { item: LogInputImageItem }) {
  ...
  const previewPanel =
    open && pos ? (
      <LogHoverTipLayer
        open={open}
        pos={{ ...pos, width: Math.min(480, pos.width) }}
        className="gw-log-preview-tip gw-log-image-preview-tip pointer-events-auto"
        ariaLabel={`${item.label} 预览`}
        tipHover={tipHover}
      >
...
export const LogImagesCell = memo(function LogImagesCell({
  ...
  if (!images.length) {
    if (hintCount != null) {
      return (
        <span className="text-sm text-[var(--gw-muted)]" title="参考图经人像库 asset:// 提交，无公网 URL">
