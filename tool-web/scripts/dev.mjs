...
if (!merged.MAIN_SITE_ORIGIN?.trim()) {
  merged.MAIN_SITE_ORIGIN = book.NEXTAUTH_URL?.trim() || "http://localhost:3000";
}
if (!merged.NEXT_PUBLIC_MAIN_SITE_ORIGIN?.trim()) {
  merged.NEXT_PUBLIC_MAIN_SITE_ORIGIN = merged.MAIN_SITE_ORIGIN;
}
if (!merged.TOOLS_PUBLIC_ORIGIN?.trim()) {
  merged.TOOLS_PUBLIC_ORIGIN = "http://localhost:3001";
}
...
if (secret.length < 16) {
  console.warn(
    "\n[tool-web] 警告: TOOLS_SSO_SERVER_SECRET 未配置或过短。\n" +
      "  SSO 回调会失败（missing_exchange_secret）。请在 book-mall/.env.local 配置后与主站一致。\n" +
      `  已尝试读取: ${bookEnvPath}\n`,
  );
}
