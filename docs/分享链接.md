...
| 入口 | 行为 |
|------|------|
| 个人中心概览 · 账户身份卡片 | 按钮「分享得积分」/ 团队主账号「分享邀请」→ 弹层；其下文字链 **邀请明细** → `/account/referral` |
| 侧栏 · 订阅与积分 | 同上 action；「邀请明细」→ `/account/referral` |
| 订阅页 `/pricing` | 「去分享」下方「分享与领取说明」→ `ShareGuideDialog`（不跳转） |

`sharePersona`：`personal` | `team_owner`（来自 `getReferralEligibility`）。

### 5.2 邀请 · 明细页（`/account/referral`）

- 异步加载 + skeleton；完整 `ReferralPanel`（邀请列表、统计）
- 8 位码（大号展示 + 复制）— **团队外拉新**，团队主账号与个人用户均可用，正常发奖
- 主站链接 `/code/{code}`
...
| 分享者 | 文案 | 行为 |
|--------|------|------|
| 个人用户 | 分享工作流 | 10 位码 + QR；正常 40 积分奖励链路 |
| 团队主账号 | 邀请成员体验 | 10 位码 + QR；claim 后尝试加入团队 · **不发**分享积分 |

API：`POST /api/platform/workflow-share` 响应含 `teamMemberShare: boolean`。

### 5.4 团队邀请（与分享码分离）

- 入口：团队中心按手机号邀请 → `/invite/t/{token}?code=`
- 用途：加成员 · 不发分享积分（非 Share Code 3.0 8/10 位码体系）
...
- [x] SC-310 团队分享分流（OWNER 工作流加成员不发奖 + UI 文案 + 邀请明细链接）
...
| 2026-08-23 | SC-310：团队 OWNER 工作流=加成员不发奖；概览按钮下邀请明细链接；sharePersona 文案 |
