...
> **状态**：已实施（2026-06-15）  
> **扣费语义 v2（2026-08）**：见 [`book-mall/doc/product/21-unified-credit-formula-v2.md`](../../book-mall/doc/product/21-unified-credit-formula-v2.md) — **人人相同 U₀**，价差在 ppc；**不再**用 `computeTierCredits` 作 Gateway 扣费。  
...
| `PER_KTOKEN` | ceil(tokens/1000) 或 **in/out 分价** | LLM |

### 1.1 LLM 输入/输出分价（2026-08-08）

百炼/DashScope LLM（含 Kimi K3）在 `ModelCostProfile` 写入 `inputListCostYuan` / `outputListCostYuan`（元/千 token），发布后对应 `inputCreditsPerKToken` / `outputCreditsPerKToken`。

结算与 Dock 预览：

```
credits = tierCredits(P_in × ceil(prompt/1000)) + tierCredits(P_out × ceil(completion/1000))
```

预览缺省估算：**4k prompt + 2k completion**。Kimi K3 百炼挂牌：输入 20 元/百万、输出 100 元/百万。
