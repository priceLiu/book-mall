...
团队列表：`GET /api/gateway/logs/dashboard/meta`
...
与财务 / BYOK 收费大类型对齐（**图表不展示 OTHER**）：

1. 文生图（含试衣）`TEXT_TO_IMAGE`
2. 图生视频 `IMAGE_TO_VIDEO`
3. 视频生视频 `VIDEO_TO_VIDEO`
4. 视频理解 `VIDEO_UNDERSTANDING`
5. TTS / 语音 `TTS`
6. 文字 `TEXT`
...
4. 轮询 20s；opportunistic `runGatewayPollWorker` + `expireStaleGatewayLogs`
...
| 方法 | 路径 | 用途 |
|------|------|------|
| GET | `/api/gateway/logs/stats` | 卡片 + 图表（无分页） |
