...
| `REDIS_URL` | 租户并发限流（轨道 D）；**未配置则并发限流全部放行**（无 Redis 也可运行） |
...
| `CANVAS_VOLCENGINE_VIDEO_TIMEOUT_MIN` | 火山 Seedance 视频 SUBMITTED 超时（分钟，默认 45） |
| `GENERATION_POLL_BATCH` | Canvas/Story poll 每轮 SUBMITTED 上限（默认 100） |
| `GENERATION_POLL_CONCURRENCY` | 单 tick 并行 poll 路数（默认 25） |
| `GENERATION_POLL_SHARD_COUNT` / `GENERATION_POLL_SHARD_INDEX` | 多实例水平分片 |
| `TEAM_MAX_CONCURRENCY_CAP` | 团队 Gateway 并发全局上限（可 >35，如 150） |
| `GENERATION_POLL_MAX_DURATION_SEC` | poll API `maxDuration`（默认 300） |
| `VIDEO_MAX_CONCURRENCY` / `VIDEO_MAX_QUEUE` | 个人账号视频并发/排队（默认 2 / 10） |
| `VIDEO_TEAM_QUEUE_MULTIPLIER` | 团队排队 = `Tenant.maxConcurrency` × 倍数（默认 5） |
| `CANVAS_PROJECT_INFLIGHT_MAX` | 单画布进行中任务上限（0=不限；生产建议 5） |
...
| 日期 | 变更 |
|------|------|
| 2026-06-19 | 并发扩展方案文档 [`docs/100人团队扩展方案.md`](../100人团队扩展方案.md)（含 20 人推荐配置）。 |
| 2026-06-19 | **50～100+ 并发视频**：poll worker 并行/多轮/分片（`GENERATION_POLL_*`）；`TEAM_MAX_CONCURRENCY_CAP`；DB 索引 `(status,lastPolledAt)`；poll route `maxDuration` 300s。 |
| 2026-06-19 | **15 人并发视频方案**：团队开通默认 `maxConcurrency=min(席位数,档位上限)`；团队视频风控对齐 `Tenant.maxConcurrency`；`GENERATION_POLL_BATCH` 默认 50；火山视频超时默认 45min；脚本 `sync-team-max-concurrency.ts`。 |
