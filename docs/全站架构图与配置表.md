...
| 日期 | 变更 |
|------|------|
| 2026-06-23 | **Gen-HotCold-R3 一小时热区**：Gateway `mode=live\|history`、主表热保留 1h（`GATEWAY_LOG_HOT_RETENTION_HOURS`）、归档 cron 小时级；gateway-web 实时/历史 + 无动停轮询；画布 `/tasks` 6h 终态窗 + 生成记录 30d。策略 `book-mall/doc/product/gen-hotcold-policy.md`。 |
