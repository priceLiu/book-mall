...
- **订单列表**：`Order` + 对订阅已支付未 `refundedAt` 的订单 **发起订阅退款审核**。  
...
2. 管理员：`completeWalletRefund`：填 **应扣未扣**（分），可选 **退款额覆盖**；默认实退 = `min(申请额或余额, 余额) - 应扣未扣`，并从钱包扣减、记 `WalletEntry` 类型 `REFUND`。  
3. `rejectWalletRefund`：`REJECTED`。

## 订阅退款审核

1. 管理员在 `/admin/billing` 订单表对订阅订单 **发起退款审核** → `SubscriptionRefundRequest`。  
