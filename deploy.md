...
**一键启动（推荐）**：在仓库根目录 `pnpm install` 后执行 `pnpm dev:all`（同时起 **3000–3008**，含电商工具箱与 quick-replica-web）；漫剧/画布本地轮询已默认包含在 `dev:all` 中。启动后打开开发导航页 http://localhost:3000/dev 。详见 [`docs/dev.md`](docs/dev.md)。
...
- `finance-web/.env.local`：`NEXT_PUBLIC_BOOK_MALL_URL=http://localhost:3000`（浏览器直连主站 API）。
...
| `tool-web` | `tool-web/` | `3001` | 推送 `main` 即触发 |
| `finance-web` | `finance-web/` | `3002` | 推送 `main` 即触发 |
...
- `canvas-web`（`canvas.ai-code8.com`）
  - `MAIN_SITE_ORIGIN` / `BOOK_MALL_URL=https://book.ai-code8.com`
  - **`TOOLS_SSO_SERVER_SECRET` / `TOOLS_SSO_JWT_SECRET` 须与 book-mall 完全一致**（缺或错会 SSO `exchange_401`）
  - `NEXT_PUBLIC_CANVAS_WEB_ORIGIN=https://canvas.ai-code8.com`
- `gateway-web`（`gateway.ai-code8.com`）
  - `GATEWAY_SSO_SERVER_SECRET`（可与 `TOOLS_SSO_SERVER_SECRET` 相同）
- `prompt-optimizer-platform`（`prompt.ai-code8.com`，端口 **3006**）
  - 同 canvas 的 `TOOLS_SSO_*` + `MAIN_SITE_ORIGIN`
  - `PROMPT_OPTIMIZER_PUBLIC_ORIGIN=https://prompt.ai-code8.com`
  - CloudBase **目标目录** `prompt-optimizer-platform`（与 canvas/story 相同）；`Dockerfile` 多阶段构建 vendor + Next，**无需**提交 `public/` 构建产物
- `finance-web`
  - `NEXT_PUBLIC_BOOK_MALL_URL=https://book.ai-code8.com`（浏览器直连主站 API，须与主站 CORS `FINANCE_WEB_ORIGINS` 一致）
...
> `finance-web/docker-entrypoint.sh` 同样会把空 `NEXT_PUBLIC_BOOK_MALL_URL` 纠正为 `https://book.ai-code8.com`。
