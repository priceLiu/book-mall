...
const API_PATH = "/api/canvas/kie/poll";

async function run() {
  console.log("[canvas-poll] start", new Date().toISOString());

  const token = process.env.STORY_AI_POLL_TOKEN;
  if (!token) {
    const msg = "STORY_AI_POLL_TOKEN not set in cloud function env";
    console.error("[canvas-poll]", msg);
  ...
  console.log("[canvas-poll] POST", url);
  ...
  console.log("[canvas-poll] done", JSON.stringify(result));
  return result;
}
