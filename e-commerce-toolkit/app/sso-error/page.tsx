...
function hintForReason(reason: string): string {
  ...
  if (reason === "missing_main_origin") {
    return "子站运行时未读到主站地址。本地请用 pnpm dev（scripts/dev.mjs）启动，或在 e-commerce-toolkit/.env.local 配置 MAIN_SITE_ORIGIN=http://localhost:3000；线上请在 e-commerce-toolkit 云托管（非 book-mall）配置 MAIN_SITE_ORIGIN=https://book.ai-code8.com。本地可访问 /api/sso-config-health 自检（生产已关闭）。";
  }
