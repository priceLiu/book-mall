...
function parseSummaryHorizontalMarketingPlans(
  ...
  const headerIdx = tableRows.findIndex(
    (r) =>
      ...
      r.some((c) => /切入/.test(c)) &&
      !/^[123]$/.test(normalizeMarketingPlanText(r[0] ?? "")),
  );
  ...
  if (headerIdx >= 0) {
    headers = tableRows[headerIdx]!;
    dataStart = headerIdx + 1;
  } else if (tableRows.some((r) => /^[123]$/.test(normalizeMarketingPlanText(r[0] ?? "")))) {
    const colCount = Math.max(...tableRows.map((r) => r.length), 6);
    headers = [
      "编号",
      "方案名称",
      "切入逻辑",
    ...
    while (headers.length < colCount) headers.push(`维度 ${headers.length + 1}`);
    dataStart = 0;
  } else {
    return [];
  }
  ...
  for (const row of tableRows.slice(dataStart)) {
    let nameRaw = normalizeMarketingPlanText(
      (nameCol >= 0 ? row[nameCol] : row[1])?.trim() ??
        row.find((c) => c.trim())?.trim() ??
        "",
    );
    if (/^编号$|^方案名称$/i.test(nameRaw)) nameRaw = "";
    ...
    if (!nameRaw) {
      const angle = rows.find((r) => /切入|逻辑/.test(r.label))?.content;
      nameRaw = angle?.slice(0, 40) || `方案 ${no}`;
    }
...
export function isAwaitingMarketingPlanSelection(
  ...
  // 仅当 Step2 方案已解析落库（或会话 Markdown 可解析出有效方案）时才进入点选态
  return resolveMarketingPlansForDisplay(project).length > 0;
}
