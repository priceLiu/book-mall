...
/** 保存种草视频工作流镜像到资产库 */
export function SeedVideoSaveDialog({
  open,
  onOpenChange,
  defaultWorkName,
  busy,
  ...
  const titlePreview = useMemo(() => {
    const base = sanitizeSaveName(name.trim() || "种草视频");
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md gap-4">
        <DialogHeader>
          ...
          <DialogDescription>
            将 Prompt、参考图、已确认脚本与成片设置镜像保存到「我的资产 → 种草视频」。
            可在资产库一键复用：复制流程后换参考图即可再生成。
          </DialogDescription>
        </DialogHeader>
