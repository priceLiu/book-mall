...
export function FilmPullMetaSummary({ meta }: Props) {
  const rows: Array<{ label: string; value: string }> = [
    { label: "总时长", value: `${meta.totalDurationSec}s` },
    { label: "叙事主线", value: meta.narrativeMainLine },
    { label: "剪辑节奏曲线", value: meta.editRhythmCurve },
    { label: "美术风格", value: meta.artStyle },
    { label: "声音设计逻辑", value: meta.audioDesignLogic },
    { label: "镜头序列逻辑", value: meta.shotSequenceLogic },
    { label: "镜头语言总结", value: meta.cameraLanguageSummary },
  ];
      ...
      <dl className="grid gap-2 sm:grid-cols-2">
        {rows.map((row) => (
          <div key={row.label} className="min-w-0">
            ...
            <dd className="mt-0.5 whitespace-pre-wrap text-sm text-[#1d1d1f]">
              {row.value.trim() || "--"}
            </dd>
          </div>
        ))}
