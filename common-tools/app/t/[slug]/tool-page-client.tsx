...
import { ImageProcessingInlineResults } from "@/components/image-processing/image-processing-inline-results";
...
import { readFileAsDataUrl } from "@/lib/image-upload-utils";
...
export function ToolPageClient({ slug }: { slug: ImageProcessingTagId }) {
  const { alert: showAlert } = useDialogs();
  const activeTag = slug;
  ...
  const [brushType, setBrushType] = useState<MaskBrushType>("round");
  ...
  const uploadError = useCallback(
    (title: string, message: string) => {
      void showAlert({ title, message, variant: "error" });
    },
    [showAlert],
  );

  const handleRetouchReplace = useCallback(async (files: File[]) => {
    const file = files[0];
    if (!file) return;
    setRetouchImage(await readFileAsDataUrl(file));
  }, []);

  const retouchReplaceDrop = useImageDropPaste({
    enabled: Boolean(retouchImage),
    onFiles: handleRetouchReplace,
    onError: uploadError,
  });
  ...
  return (
    <div className="min-h-0 flex-1">
      <div className="ecom-scrollbar-thin min-h-0 flex-1 overflow-y-auto px-4 py-4 sm:px-6 sm:py-6 lg:px-8 lg:py-8">
          ...
          <h1 className="mt-4 text-2xl font-bold tracking-tight text-[#1d1d1f] sm:mt-6 sm:text-3xl">
          ...
          {panel === "placeholder" ? (
            <div className="mt-6 rounded-2xl border border-dashed border-[#d2d2d7] bg-white p-8 text-center sm:mt-8 sm:p-12">
              ...
              <Link href="/" className="mt-6 text-sm text-[#0071e3] hover:underline">
                返回工具菜单
              </Link>
            </div>
          ) : panel === "retouch" ? (
            <div className="mt-6 space-y-4 sm:mt-8 sm:space-y-6">
              ...
              <div className="rounded-2xl border border-[#e5e5ea] bg-white p-4 shadow-sm sm:p-6">
                {!retouchImage ? (
                  ...
                  <div
                    {...retouchReplaceDrop.dropZoneProps}
                    className={cn(
                      "rounded-xl transition-colors",
                      retouchReplaceDrop.dragOver && "ring-2 ring-violet-400 ring-offset-2",
                    )}
                  >
                    <MaskToolbar
                      brushSize={brushSize}
            ...
            <div className="mt-6 space-y-4 sm:mt-8 sm:space-y-6">
              ...
              <div className="rounded-2xl border border-[#e5e5ea] bg-white p-4 shadow-sm sm:p-6">
                {supportsMultiImageEditor(editorModel) ? (
                  ...
                  <ImageSingleUpload
                    image={editorImages[0] ?? null}
                    onChange={(url) => setEditorImages(url ? [url] : [])}
                    onError={uploadError}
                    emptyLabel="将图片拖放到此处 或点击浏览"
                    icon={CloudUpload}
                  />
