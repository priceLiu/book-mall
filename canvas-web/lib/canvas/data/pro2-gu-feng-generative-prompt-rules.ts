...
export const PRO2_GU_FENG_CHARACTER_IMAGE_RULES = `# 古风 · 角色生图规范（写入「外貌/服装」列 · 三视图【角色设定】块）

- **语言**：列内 **默认写中文** bullet 列表（以「- 」开头）；表头含「英文」仅为系统解析兼容，**非必要禁止英文**
- **字段**：推荐「年龄与身份 / 发型 / 面容 / 服饰 / 配饰或肩甲 / 整体材质」；**只写造型**（是什么），不写摄影画风、光线、场景与道具
- **严格锁定（最高优先级）**：五官、发型发饰、服装款式与主色须与角色视觉辞典/用户确认形象 **完全一致**；剧本性格、情节氛围 **不得** 改变外观；仅用户明确要求才可局部改
- **禁止**：性格描述、背包/手提物、手持或挡脸动作；禁止在角色列写「写实电影感/2K/自然光」等（这些由大纲视觉风格总纲 → 【全局视觉风格】块注入）
- **三视图下游**：系统会自动套【任务】【硬性约束】与【全局视觉风格】；角色列只负责【角色设定】bullet
- **用户固定形象**：若类别参考或角色辞典已给出完整 bullet，下游须 **全文采用**，不得因剧本情节改写五官/发型/服装配色`;
