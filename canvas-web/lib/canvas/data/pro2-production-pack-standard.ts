...
/** 摄影级视觉风格 · JSON visualStyle 字段 */
export const STORY_PRO2_VISUAL_STYLE_TABLE_RULES_V6 = `- **visualStyle** 对象须 **具体可执行**（禁止空泛「高质量」「精美」）：

| JSON 字段 | 内容要求 |
|-----------|----------|
| worldBackground | 世界观 / 时代背景 / 戏剧空间，1–3 句 |
| era | 年代 + 地点 + 季节/气候 |
| globalColorTone | 主色名 + HEX，日/夜或冷暖对比概述 |
| pictureStyle | 国风二次元厚涂，2D动漫媒介（须与角色/场景/道具视觉风格保持一致） |
| cinematography | 镜头焦段、景深、光比、构图习惯 |
| dayPalette | 对象 { primary?, highlight?, shadow? }；无日景则省略 |
| nightPalette | 对象 { primary?, highlight?, shadow? }；无夜景则省略 |
| skinMaterial | 主要角色肤色或材质倾向，可选 HEX |
| setDesign | 建筑/环境材质与主色，1–2 句 |
| lighting | 自然光方向、轮廓光、拒绝平光等 |
| styleAnchor | 中文风格锚定优先；非必要不写英文，可 prepend 到生图 prompt |

- 后续三视图/场景/道具/分镜节点将自动读取 visualStyle。
- **画面风格统一**：所有角色、场景、道具的 imagePrompt 末尾须追加 \`[视觉风格：xxx]\`，与 styleAnchor / pictureStyle 一致。`;

/** 核心冲突与结构摘要 · JSON coreConflict[] */
export const STORY_PRO2_CORE_CONFLICT_TABLE_RULES = `- **coreConflict[]** 须为 \`{ dimension, content }\` 对象数组，至少包含：

| dimension | content 要求 |
|-----------|----------------|
| 表层/深层冲突 | 外部冲突 + 内心诉求 |
...
- 禁止仅用散文段落代替 coreConflict[]；信息量须足以支撑 **12–18 镜**分镜拆分。`;

/** 下一步交接清单 · JSON handoff[] */
export const STORY_PRO2_HANDOFF_TABLE_RULES = `- **handoff[]** 须为 \`{ index, item, owner, note }\` 对象数组，至少 6 项，覆盖：角色三视图、场景图、**道具六视图**、分镜 Pass2 提示词、分镜视频、对白配音、BGM/音效、剪辑交付等；note 须写可执行细节。`;
...
/** 分镜段 LLM 专用 · 单镜结构参考（减 token；完整 4 镜见 PRO2_DEFAULT_SHOT_GFM_EXAMPLE · 仅人类可读参考） */
export const PRO2_STORYBOARD_FEW_SHOT_COMPACT = `${PRO2_CANVAS_PASS1_SHOT_FIELD_GUIDE}

【分镜段任务】须输出 **完整镜头序列**（**12–18 镜** · 总时长 175–185 秒）；**每镜必填 durationSec 整数 10–15**；禁止只输出 1–2 镜样例即停。
**Pass1 禁止** 输出 imagePrompt / videoPrompt / frameImagePrompt；分镜图/视频提示词由 Pass2「生成提示词」完成。

**单镜 JSON 结构参考（仅 1 镜 · 禁止照抄剧情 · 须依大纲续写至目标镜数）**：
...
\`\`\`json
{
  ...
  "cameraMove": "固定机位，微小手持晃动增加压抑感",
  "sceneDescription": "【起始】在伏案加班…【结束】保持伏案姿势，视线锁定屏幕",
  "propIds": ["prop-computer"],
  "dialogue": "—",
  "durationSec": 5,
  "sfxNote": "急促而沉重的键盘敲击声，微弱的空调底噪",
