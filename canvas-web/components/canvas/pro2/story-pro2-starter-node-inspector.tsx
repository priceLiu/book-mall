...
import {
  ...
  PRO2_INSPECTOR_ACTION_BTN_CLASS,
  PRO2_TEMPLATE_CHIP_CLASS,
  PRO2_TEMPLATE_CHIP_SELECTED_CLASS,
...
} from "@/lib/canvas/story-pro2-starter-sync";
import {
...
import { resolveStoryProStarterScriptInput } from "@/lib/canvas/story-pro2-starter-text";
import type { StoryProScriptHubNodeData, StoryProStarterMode } from "@/lib/canvas/story-pro-workspace-types";
import type { StoryPro2StarterInspectorData } from "@/lib/canvas/story-pro2-workspace-types";
...
export function StoryPro2StarterInspector({ id, data, selected }: NodeProps) {
  ...
  const d = data as unknown as StoryPro2StarterInspectorData;
  ...
  const scriptHub = useMemo(
    () => findStoryPro2ScriptHubForStarter(nodes, edges, id, d.workspaceIds),
  ...
  useEffect(() => {
    ...
    if (!ws) return;
    const patch: Partial<StoryPro2StarterInspectorData> = {};
  ...
  useEffect(() => {
    ...
    void fetchUploadedScriptFromOss(base, d.uploadedScriptOssUrl)
      ...
      .catch((e) => {
        if (!cancelled) {
          console.warn("[story-pro2-starter] load script from OSS failed", e);
        }
      })
      .finally(() => {
  ...
  const ensureScriptMdLoaded = async (): Promise<boolean> => {
    const latest = useCanvasStore.getState().nodes.find((n) => n.id === id)
      ?.data as StoryPro2StarterInspectorData | undefined;
  ...
  const onParseScript = async () => {
    ...
    if (!ready) return;
    const state = useCanvasStore.getState();
    const existingHub = findStoryPro2ScriptHubForStarter(
      state.nodes,
      state.edges,
      id,
      d.workspaceIds,
      ...
      <ProNodeShell
        ...
        footer={
          <div className="nodrag flex w-full flex-col gap-2">
            <div className="shrink-0 space-y-1.5 border-b border-violet-400/10 pb-2">
              <p className={PRO2_HINT_LABEL_CLASS}>LLM 模型（全工作流共用）</p>
              <div
                className={
            ...
            <StoryNodeFooterShell
              hint={
                <span className="flex items-center gap-1 text-violet-200/60">
      ...
      >
        <div className="flex h-full min-h-0 flex-col gap-2.5">
          <div className="flex flex-wrap gap-1">
            <button
              type="button"
              className={`${PRO2_TEMPLATE_CHIP_CLASS} border-violet-400/45 bg-violet-500/20`}
            >
              上传剧本
            </button>
            <button
              ...
              className={`${PRO2_TEMPLATE_CHIP_CLASS} opacity-40`}
            >
              创作剧本 · 敬请期待
            </button>
          </div>
          ...
          <div
            className={`space-y-1 rounded-md transition ${
              dragOver ? "ring-2 ring-violet-400/50 ring-offset-1 ring-offset-black/40" : ""
            }`}
            onDragOver={(e) => {
              e.preventDefault();
              e.stopPropagation();
            ...
            onDrop={onScriptDrop}
          >
            <span className={PRO2_HINT_LABEL_CLASS}>
            ...
            <div className="flex flex-wrap gap-1.5">
              <button
                ...
                className={`nodrag inline-flex min-h-[2.75rem] flex-1 items-center justify-center gap-1.5 rounded border border-dashed px-2 py-2 text-[11px] transition disabled:cursor-not-allowed disabled:opacity-45 ${
                  dragOver
                    ? PRO2_TEMPLATE_CHIP_SELECTED_CLASS
                    : "border-violet-400/30 bg-violet-500/8 text-violet-100/90 hover:border-violet-400/50 hover:bg-violet-500/14"
                }`}
              >
              ...
              {hasUploadedScript ? (
                <>
                  <button
                    ...
                    className="nodrag rounded border border-violet-400/25 bg-black/30 px-2 py-2 text-[11px] text-violet-100/90 hover:bg-violet-500/10 disabled:opacity-45"
            ...
            {starterMode === "upload" && !hasUploadedScript && !upstreamScript ? (
              <p className="text-[10px] leading-relaxed text-violet-200/45">
          ...
          <div className="min-h-0 flex-1 space-y-1 overflow-hidden">
            <p className={PRO2_HINT_LABEL_CLASS}>
              导演提示词 · 输入 @ 引用上传剧本
            </p>
